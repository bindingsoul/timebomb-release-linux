#!/bin/bash
# dlt-5-min.sh: Schedule files/folders for deletion in 5 minutes with error handling

set -euo pipefail

DELETE_EPOCH=$(date -d "+5 minutes" +%s)
...
for f in "$@"; do
  ...
  if jq --arg f "$f" --argjson e "$DELETE_EPOCH" \
    ...
    echo "[$(date)] Scheduled for deletion: $f (in 5 minutes)" | tee -a "$LOG_FILE"
    echo "💣 Scheduled for deletion: $f (in 5 minutes)"
  else
