#!/bin/bash
# dlt-1-month.sh: Schedule files/folders for deletion in 1 month with error handling

set -euo pipefail

DELETE_EPOCH=$(date -d "+1 month" +%s)
...
for f in "$@"; do
  ...
  if jq --arg f "$f" --argjson e "$DELETE_EPOCH" \
    ...
    echo "[$(date)] Scheduled for deletion: $f (in 1 month)" | tee -a "$LOG_FILE"
    echo "💣 Scheduled for deletion: $f (in 1 month)"
  else
