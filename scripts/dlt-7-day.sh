#!/bin/bash
# dlt-7-day.sh: Schedule files/folders for deletion in 7 days with error handling

set -euo pipefail

DELETE_EPOCH=$(date -d "+7 days" +%s)
...
for f in "$@"; do
  ...
  if jq --arg f "$f" --argjson e "$DELETE_EPOCH" \
    ...
    echo "[$(date)] Scheduled for deletion: $f (in 7 days)" | tee -a "$LOG_FILE"
    echo "💣 Scheduled for deletion: $f (in 7 days)"
  else
