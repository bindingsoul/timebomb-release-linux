#!/bin/bash
# dlt-1-day.sh: Schedule files/folders for deletion in 1 day with error handling

set -euo pipefail

DELETE_EPOCH=$(date -d "+1 day" +%s)
...
for f in "$@"; do
  ...
  if jq --arg f "$f" --argjson e "$DELETE_EPOCH" \
    ...
    echo "[$(date)] Scheduled for deletion: $f (in 1 day)" | tee -a "$LOG_FILE"
