#!/bin/bash
# dlt-1-year.sh: Schedule files/folders for deletion in 1 year with error handling

set -euo pipefail

DELETE_EPOCH=$(date -d "+1 year" +%s)
...
for f in "$@"; do
  ...
  if jq --arg f "$f" --argjson e "$DELETE_EPOCH" \
    ...
    echo "[$(date)] Scheduled for deletion: $f (in 1 year)" | tee -a "$LOG_FILE"
    echo "💣 Scheduled for deletion: $f (in 1 year)"
  else
